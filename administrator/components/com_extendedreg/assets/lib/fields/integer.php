...
class erFieldInteger extends erField implements erFieldInterface {
	...
	public function getHtml($value, $id = null) {
		...
		return '<input' . (trim($class) ? ' class="' . $class . '"' : '') . (trim($id) ? ' id="' . $id . '"' : '') . ' type="text" name="' . $this->_fld->name . '" value="' . (mb_strlen(trim($value)) ? (int)$value : '') . '" />';
	}
}